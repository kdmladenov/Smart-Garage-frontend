...
const validate = {
  notes: value => typeof value === 'string' && value.length > visitInput.NOTES_MIN_LENGTH && value.length < visitInput.NOTES_MAX_LENGTH,
  visitEnd: value => typeof value === 'string' && !(new Date(value).toString()).includes('Invalid'),
  status: value => Object.values(visitStatusEnum).includes(value)
};
